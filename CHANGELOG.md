...
* wip... `240228piu01` created a mechanism to run adhoc commands *for xl2roefact component*:
    ...
    * [x] 3. workflow `adhoc-run.yml` to install pdm environment & run `./tests/adhoc.cmd &>./tests/_test_results.txt` (with crt directory `xl2roefact/`) triggered by merge to branch `adhoc`. Run results (stdout) written to `./tests/_test_results.txt`
