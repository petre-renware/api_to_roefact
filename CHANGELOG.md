...
* wip... `owner-dbs-file` get OWNER EXTERNAL DATA feature:
    ...
    * [ ] code to get owner data (using function `libutil.hier-get-data-file()`)
...
* wip... `hier-get-data-file` func to select hierarchical a file from `./` or `data/` (in libutils module):
    * [ ] ...1. create skeleton `hier-get-data-file(file_name: str) -> pathlib.Path` in `xl2roefact.libutils` module
