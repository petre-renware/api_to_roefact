...
* wip... `240228piu01` created a mechanism to run adhoc commands *for xl2roefact component*:
    ...
    * [x] 2. skeleton windows-cmd-file to be run by adhoc pdm script (ref step 1) usable as template `xl2roefact/tests/adhoc.cmd` (NOTE: running OS = Windows)
    * [x] 3. workflow `adhoc-run.yml` to install pdm environment & run `./tests/adhoc.cmd &>./tests/_test_results.txt` (with crt directory `xl2roefact/`) triggered by merge to branch `adhoc`. Run results (stdout and stderr) written to `./tests/_test_results.txt`
